...
\begin{figure}[H]
	\noindent \fbox{
		\begin{minipage}{0.96\textwidth}
			\textbf{Learning}
			\begin{enumerate}
				\item Make a pair of problem text, $p$ and solution, $l$
				\item Covert to fraction as (1)
				\item Tokenize, replace and ground $p$ to base Qsets, $S$
				\item $T_{i}$  is the generated top $M$ candidate trees by $ILP$
				\item 5. $T_{l_i} \leftarrow$ Select best equation trees based on the solution, $l$
				\item Extract Local Model features for $Qset <s_1, s_2>$ labeled with $op$
				\item Extract Global Model features by $T_i$  and solution of $T_i$ labeled with positive or negative
				\item $L_{local} \leftarrow$  Train Local Model labeled with operator
				\item $G_{global} \leftarrow$  Train Global Model labeled with Positive or negative
			\end{enumerate}
		\end{minipage}
	}
	...
	\noindent \fbox{
		\begin{minipage}{0.96\textwidth}
			\textbf{Inference}
			\begin{enumerate}
				\item Step 1 – 7 from Learning
				\item Select best equation based on the score from local and global training model like (2)
				\item Evaluate the equation
